...
<div class="container">
    <h3 class="text-center mb-4">PERMINTAAAN PERBAIKAN</h3>
    <div class=" pb-3 d-flex align-items-center justify-content-between">
        <?php if ($this->session->id == 1 || $this->session->id == 2) : ?>
            <a href="<?= base_url('aplikasi/permintaan/create/') ?>" class="btn btn-primary"><i class="fa fa-plus"></i> Permintaan</a>
        <?php endif; ?>
        <form method="post" class=" d-md-inline-block form-inline ml-auto mr-0 mr-md-3 my-2 my-md-0">
            <div class="input-group">
                ...
                <div class="input-group-append">
                    ...
                    <a href="<?= base_url('/aplikasi/permintaan/reset') ?>" class="btn btn-warning">RESET</a>
                </div>
            </div>
        </form>
    </div>
    <?php if (empty($permintaan)) : ?>
        <div class="alert alert-danger" role="alert">
            Data tidak ditemukan ! Reset pencarian atau buat permintaan perbaikan baru.
        </div>
    <?php endif; ?>
                ...
                <div class="card mb-4">
                    <div class="card-body">
                            ...
                            <?php if ($this->session->id == 1) : ?>
                                <div class="pl-2">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="small dropdown-item" href="<?= base_url('aplikasi/permintaan/edit/' . $data->id) ?>"><i class="fa fa-edit"></i> Edit</a>
                                    </div>
                                </div>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="d-flex align-items-center justify-content-between">
                            <p class="small"><?= $data->tanggal; ?> | <?= $data->jam; ?></p>
                            <p class="small">Status : <span class="badge  <?= $data->status == 'Selesai' ? 'badge-success' : 'badge-primary' ?> "><?= $data->status; ?></span></p>
                        </div>
                    </div>
                    <div class="card-footer d-flex align-items-center justify-content-between">
                        <a class="small" href="<?= base_url('aplikasi/kemajuanpermintaan/view/' . $data->id); ?>">Laporan Kemajuan <i class="fa fa-arrow-right"></i></a>
                        <div class="small ">
                            <?php if ($this->session->id == 1) : ?>
                                <a onclick="deleteConfirm('<?php echo site_url('aplikasi/permintaan/delete/' . $data->id) ?>')" href="#!"><i class="text-danger fa fa-trash"></i></a>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
